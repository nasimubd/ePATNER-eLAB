...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-purple-50 p-2 sm:p-4">
    <div class="max-w-7xl mx-auto">
        {{-- Enhanced Header --}}
        <div class="bg-white/80 backdrop-blur-sm shadow-xl rounded-2xl overflow-hidden border border-white/20 mb-4">
            <!-- Header Section -->
            <div class="bg-gradient-to-r from-purple-600 via-purple-700 to-indigo-700 p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="text-center sm:text-left">
                        ...
                        <p class="text-purple-100 text-sm">Manage and monitor your hospital patients efficiently</p>
                    </div>

                    <div class="w-full sm:w-auto flex gap-3">
                        <a href="{{ route('admin.patients.export', request()->query()) }}"
                            class="bg-white text-purple-600 hover:bg-gray-50 px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center justify-center shadow-md hover:shadow-lg">
                            <i class="fas fa-file-export mr-2"></i>
                            <span class="hidden sm:inline">Export Data</span>
                            <span class="sm:hidden">Export</span>
                        </a>
                        <a href="{{ route('admin.patients.create') }}"
                            class="bg-yellow-400 text-gray-900 hover:bg-yellow-300 px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center justify-center shadow-md hover:shadow-lg">
                            <i class="fas fa-plus mr-2"></i>
                            <span class="hidden sm:inline">Add New Patient</span>
                            <span class="sm:hidden">Add Patient</span>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 bg-gradient-to-r from-gray-50 to-purple-50 border-t border-gray-200">
                <form method="GET" action="{{ route('admin.patients.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div class="lg:col-span-2">
                        <label for="search" class="block text-sm font-medium text-gray-700 mb-2">Quick Search</label>
                        <div class="relative">
                            <input type="text" id="search" name="search" value="{{ request('search') }}"
                                placeholder="Patient ID, Name, Phone..."
                                class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="w-full py-2.5 px-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                    ...
                    <div class="flex items-end gap-2">
                        <button type="submit" id="filterBtn"
                            class="flex-1 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-medium py-2.5 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-md text-sm">
        ...
        <div class="lg:hidden space-y-3 mt-4">
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 class="font-semibold text-gray-900 text-sm truncate max-w-xs" title="{{ $patient->first_name }} {{ $patient->last_name ?? '' }}">
                                    {{ $patient->first_name }} {{ $patient->last_name ?? '' }}
                                </h3>
                                <p class="text-xs text-gray-500 truncate max-w-xs" title="{{ $patient->occupation ?? 'Patient' }}">
                                    {{ $patient->occupation ?? 'Patient' }}
                                </p>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ ($patient->is_active ?? true) ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                    ...
                    <div class="grid grid-cols-1 gap-4 text-sm text-gray-700">
                        <div><strong>ID:</strong> {{ $patient->patient_id ?? 'N/A' }}</div>
                    ...
                    <div class="flex justify-between mt-4 space-x-2">
                        <!-- <a href="{{ route('admin.patients.show', $patient) }}"
                            class="flex-1 inline-flex items-center justify-center p-2 rounded-lg bg-purple-50 text-purple-600 hover:bg-purple-100 transition-all duration-300">
                        ...
                        <form action="{{ route('admin.patients.destroy', $patient) }}" method="POST" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            <div class="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-8 text-center mt-6">
                ...
                <p class="text-gray-500 text-sm">Try adjusting your search criteria or create a new patient.</p>
            </div>
            @endforelse
        </div>
        ...
        <div class="hidden lg:block bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-white/20 overflow-hidden mt-4">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-gray-50 to-purple-50 border-b border-gray-200">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                <i class="fas fa-hashtag mr-2"></i>ID
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                <i class="fas fa-user-injured mr-2"></i>Name
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Phone
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-purple-50 hover:to-indigo-50 transition-all duration-300">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $patient->patient_id ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex justify-center space-x-2">
                                    <!-- <a href="{{ route('admin.patients.show', $patient) }}" title="View"
                                    ...
                                    <a href="{{ route('admin.patients.edit', $patient) }}" title="Edit"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (filterBtn) {
            filterBtn.addEventListener('click', function() {
                this.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i><span class="hidden sm:inline">Searching...</span>';
                this.disabled = true;
            });
        }
