...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
        <div class="flex items-center justify-between bg-gray-100 px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-700">Edit Care Of</h3>
        ...
        <div class="p-6">
            ...
            <div class="mb-6 bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded relative" role="alert">
                <div class="flex items-center">
                    ...
                    <span>Updating this Care Of will also update the associated ledger information.</span>
                </div>
            </div>

            <form action="{{ route('admin.care-ofs.update', $careOf) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text"
                            id="phone_number"
                            name="phone_number"
                            value="{{ old('phone_number', $careOf->phone_number) }}"
                            required
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="3"
                        required
                        class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 @error('address') border-red-500 @enderror">{{ old('address', $careOf->address) }}</textarea>
                    @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="commission_type"
                                ...
                                <option value="percentage" {{ old('commission_type', $careOf->commission_type) == 'percentage' ? 'selected' : '' }}>Percentage</option>
                                <option value="fixed" {{ old('commission_type', $careOf->commission_type) == 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                            </select>
                        ...
                        <div id="percentage_field" style="display: none;">
                            ...
                            <input type="number"
                                ...
                                value="{{ old('commission_rate', $careOf->commission_rate) }}"
                                step="0.01"
                                min="0"
                                max="100"
                        ...
                        <div id="fixed_field" style="display: none;">
                            ...
                            <input type="number"
                                ...
                                value="{{ old('fixed_commission_amount', $careOf->fixed_commission_amount) }}"
                                step="0.01"
                                min="0"
                ...
                <div>
                    ...
                    <select id="status"
                        ...
                        <option value="active" {{ old('status', $careOf->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $careOf->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-wrap gap-4 mt-6">
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <i class="fas fa-save mr-2"></i> Update Care Of
                    </button>
